...
class MainWindow(QWidget):
        def __init__(self, parent=None):
                ...
                self.userDetailButton.clicked.connect(self.userDetail)
                ...
                MainWindow.userDetail = QPlainTextEdit()
                MainWindow.userDetail.setWindowTitle('User Details')
                MainWindow.userDetail.resize(350, 550)
                #
                self.userDetail = QPlainTextEdit()
                self.userDetail.setWindowTitle('User Details')
                self.userDetail.resize(350, 550)
                # Set layout
                #
                grid = QGridLayout()
                self.setLayout(grid)
                ...
                grid.addWidget(self.userEdit, 2, 1, 1, 2)
        ...
        @Slot()
        def userDetail(self):
            print "start userDetail"
            if MainWindow.userDetail.isVisible():
                print "its visible 2nd"
                self.userDetail.hide()
            else:
                print "not visible"
                MainWindow.userDetail.move(self.pos())
                # OMG such a hack job to move the window
                newstring = str(self.pos())
                x, y = newstring.split(",", 1)
                x = x[21:]
                ...
                x = int(x) + 381
                y = int(y)
                MainWindow.userDetail.move(x, y)
                MainWindow.userDetail.show()
