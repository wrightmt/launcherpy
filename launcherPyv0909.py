    ...
    def run(self):
        ...
        while self.exiting == False:
            ...
            if logonProc in newProcList:
                MainWindow.lockedStatus = 1
                #MainWindow.statusBar.showMessage("Online - Locked")
            else:
                MainWindow.lockedStatus = 0
...
class commandMessageThread(QThread):
    ...
    def run(self):
        while self.exiting == False:
            ...
            sendMessage = subprocess.Popen(["powershell", "-ExecutionPolicy", "Unrestricted", ".\message.ps1", "-Message", '"' + MainWindow.commandMessageData + '"', "-TargetPC", MainWindow.hostCombo.currentText()], shell=True, stdout=subprocess.PIPE)
            sendMessage.wait()
            self.signal.sig.emit('OK')
            self.exiting = True
        self.exiting = False
...
class commandRemoteThread(QThread):
    ...
    def run(self):
        #MainWindow.statusBar.showMessage('Checking for service')
        while self.exiting == False:
            print "command remote started"
            pythoncom.CoInitialize()
            if MainWindow.commandRemoteAuto.isChecked() == True:
                procList = list()
                hostLogonCheck = wmi.WMI(MainWindow.hostCombo.currentText())
                for process in hostLogonCheck.Win32_Process():
                    checkProcessName = process.Name
                    procList.append(checkProcessName)
                newProcList = str(procList)
                ...
                if logonProc in newProcList:
                    ...
                    #MainWindow.statusBar.showMessage('Opening remote session')
...
class computerModelThread(QThread):
    ...
    def run(self):
        ...
        while self.exiting == False:
            ...
            historyHostItem = QTableWidgetItem(readHostThread.currentHost.upper())
...
class MainWindow(QWidget):
        def __init__(self, parent=None):
                ...
                self.hostCombo.lineEdit().returnPressed.connect(self.readHostOperation)
                ...
                self.alwaysOnTop.stateChanged.connect(self.alwaysOnTopSet)
                self.alwaysOnTopLabel = QLabel('On Top')
                # logon check
                MainWindow.checkLogon = checkLogonThread()
                MainWindow.checkLogon.signal.sig.connect(self.checkLogonComplete)
                # dumb ping process
                #
                ...
                self.commandRemoteLabel = QLabel('Service Check')
                MainWindow.commandRemoteAuto = QCheckBox()
                self.commandRemoteLabel.setAlignment(Qt.AlignLeft)
                #
                ...
                palette = QPalette()
                pixmap = QPixmap("tomago_logo.png")
                brush = QBrush(pixmap)
                ...
                grid.addWidget(self.alwaysOnTop, 1, 1)
                grid.addWidget(self.alwaysOnTopLabel, 1, 0)
                ...
                grid.addWidget(MainWindow.commandRemoteAuto, 10, 1)
                grid.addWidget(self.commandRemoteLabel, 10, 0)
        ...
        def checkLogonComplete(self):
            if MainWindow.lockedStatus == 1:
                MainWindow.statusBar.showMessage("Online - Locked")
        ...
        def commandRemote(self):
            if not self.commandRemoteThread.isRunning():
                ...
                #self.statusBar.showMessage('Checking for remote service')
                #self.readHostButton.setEnabled(False)
